...
int main() {
  ...
  while (window_top.isOpen() && window_side.isOpen() && swarm.size() > 1) {
    ...
    window_top.clear();
    boids::drawBoids(swarm, predator, window_top, 0, boid_shape, predator_shape);
    window_top.display();

    window_side.clear();
    boids::drawBoids(swarm, predator, window_side, 1, boid_shape, predator_shape);
    window_side.display();

    boids::updateSimulation(predator, swarm, t, "boids_save.txt");
